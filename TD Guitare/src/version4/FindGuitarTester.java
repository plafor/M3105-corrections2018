...
public class FindGuitarTester {
	...
	public static void main(String[] args) {
		...
		GuitarSpec whatErinLikes = new GuitarSpec(Builder.FENDER, "Stratocastor", Type.ELECTRIC, 6, Wood.ALDER,
				Wood.ALDER);
		List<Guitar> lesGuitars = inventory.search(whatErinLikes);

		if (!lesGuitars.isEmpty()) {

			for (Guitar uneGuitar : lesGuitars) {
				System.out.println("Erin, you might like this " + uneGuitar.getSpecs().getBuilder() + " "
						+ uneGuitar.getSpecs().getModel() + " " + uneGuitar.getSpecs().getType() + " guitar:\n   "
						+ uneGuitar.getSpecs().getBackWood() + " back and sides,\n   "
						+ uneGuitar.getSpecs().getTopWood() + " top.\nYou can have it for only $" + uneGuitar.getPrice()
						+ "!");
			}
