...
public class FindGuitarTester {
	...
	public static void main(String[] args) {
		...
		InstrumentSpec whatErinLikes = new GuitarSpec(Builder.FENDER, "Stratocastor", 6, Type.ELECTRIC, Wood.ALDER,	Wood.ALDER);
		List<Instrument> lesInstruments = inventory.search(whatErinLikes);

		if (!lesInstruments.isEmpty()) {

			for (Instrument aFoundInstrument : lesInstruments) {
				System.out.println("  - " + aFoundInstrument);
			}

		} else {
			System.out.println("Pas de correspondances.");
		}
	}
